...
from .permissions import IsOwnerOrReadOnly  # Осы файл бар екеніне көз жеткіз

# --- POSTS CRUD ---

@api_view(['GET', 'POST'])
@permission_classes([IsAuthenticatedOrReadOnly])
@parser_classes([MultiPartParser, FormParser]) # Сурет пен Текстті қатар қабылдау үшін
def post_list_create(request):
    if request.method == 'GET':
        # Барлық посттарды базадан алу
        posts = Post.objects.all().order_by('-created_at')
        serializer = PostSerializer(posts, many=True)
        return Response(serializer.data)
        
    elif request.method == 'POST':
        # 1. Постты (текст) локальді SQLite базасына сақтау
        serializer = PostSerializer(data=request.data)
        if serializer.is_valid():
            # Пост авторын автоматты түрде қазіргі юзер ретінде бекіту
            post = serializer.save(author=request.user)
            
            # 2. Суретті (медиа) локальді /media/ папкасына сақтау
            image_file = request.FILES.get('media')
            if image_file:
                # Media моделі арқылы суретті файлдық жүйеге сақтаймыз
                Media.objects.create(post=post, file=image_file)
            
            # Сақталған постты қайтару (ішіндегі медиаларымен бірге)
...
def post_detail(request, pk):
    ...
    # Авторды тексеру (Permissions автоматты түрде істемесе, қолмен тексеру)
    ...
    if request.method == 'GET':
        ...
        # Постты жартылай жаңарту (partial=True)
        serializer = PostSerializer(post, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            
            # Егер жаңа сурет жіберілсе, оны қосу
            image_file = request.FILES.get('media')
            if image_file:
                Media.objects.create(post=post, file=image_file)
